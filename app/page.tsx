            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={inv.id} className={`border-b border-zinc-100 hover:bg-zinc-50 ${i % 2 === 0 ? "" : "bg-zinc-50"}`}>
                  ...
                  <td className="px-4 py-3 text-right text-zinc-700">{fmt(inv.sub_total, inv.currency_code)}</td>
...
function ClientModule({ client, onBack }: { client: Client; onBack: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 p-6 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={onBack} className="text-zinc-500 hover:text-zinc-700 transition text-sm">← All Clients</button>
        ...
        {/* Sub-module tabs */}
        <div className="flex gap-1 bg-white border border-zinc-200 rounded-xl p-1 w-fit overflow-x-auto">
          {SUB_MODULES.map(m => (
            <button key={m.key} onClick={() => setActiveTab(m.key)}
              className={`text-sm px-4 py-1.5 rounded-lg transition whitespace-nowrap ${activeTab === m.key ? "bg-zinc-200 text-zinc-900" : "text-zinc-500 hover:text-zinc-700"}`}>
              {m.label}
            </button>
          ))}
        </div>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onLogout}
            className="text-xs px-3 py-1.5 rounded-md bg-zinc-100 hover:bg-red-900 text-zinc-700 hover:text-red-300 transition">
            Sign Out
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {clients.map(client => (
              <button key={client.id} onClick={() => setActiveClientId(client.id)}
                className="text-left bg-white border border-zinc-200 rounded-xl p-6 hover:border-zinc-400 hover:bg-zinc-100 transition group">
